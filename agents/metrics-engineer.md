...
Read `VENTURE.md` to find the OMTM and `## Источник метрик`. Then:
...
2. **Build the collector.** Prefer generating `scripts/metrics/collect.sh` (or `collect.py`) **at the
   ...
   - Where you have **no access** (a manual export, a console with no API), instead write **exact
     ...
     from a known path.
3. **Register the source** in `VENTURE.md → ## Источник метрик`: type (`script | file | API | none`),
...
Content you write into `VENTURE.md` and any instructions to the user are in **Russian**.
